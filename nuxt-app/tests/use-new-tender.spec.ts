...
import { useNewTender } from '../app/composables/useNewTender'
...
describe('useNewTender', () => {
  it('starts with the expected default state', () => {
    const { activeStep, tenderData, totalWeight, isStepValid } = runInScope(() => useNewTender())
  ...
  it('allows navigating only when the current step is valid', () => {
    const { activeStep, tenderData, nextStep, isStepValid } = runInScope(() => useNewTender())
  ...
  it('resets the wizard state to its initial values', () => {
    const { activeStep, tenderData, addCriterion, addPriceBlock, addVendor, nextStep, reset } = runInScope(() => useNewTender())
